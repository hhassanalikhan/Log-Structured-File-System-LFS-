...
static int hello_getattr(const char *path, struct stat *stbuf)
{
    ...
    if (strcmp(path, "/") == 0) {
        ...
        stbuf->st_nlink = 1;
        stbuf->st_size = 96090;//sizeof(hello_str);
    }
...
struct fuse_file_info *fi)
{
   ...
   FILE *fp;
    fp = fopen("/home/osboxes/Desktop/dir2", "rb+");
...
if(g != -1)
	...
	int lit = data[sz11-4]-'0';

        lit = lit*10 +data[sz11-3]-'0';
        lit = lit*10 +data[sz11-2]-'0';
        lit = lit*10 +data[sz11-1]-'0';
...
fp = fopen("/home/osboxes/Desktop/d52", "r");
if(fp == NULL)
	y = -10;
else
{
	...
	for(z = 0;z<8;z++ )
	{
		...
		for (v = 0; v < 4 ; v++)
		{
			nu[v] = data[(lit*1024)+(j*40)+ 4 + (z*4) + v];//  inodes,inode numb,  num of blks read ,char to be read
		}
		//y++;
		int j11 = 0;
...
off_t offset, struct fuse_file_info *fi)
{
    ...
    if(ind == 1024)
    {
        ...
        fp = fopen("/home/osboxes/Desktop/dir2", "w");
        fprintf(fp, "%s", data);
    ...
    if(g != -1)
    ind = g;
    int lit = data[sz11-5]-'0';
    lit = lit*10 +data[sz11-4]-'0';
    lit = lit*10 +data[sz11-3]-'0';
    lit = lit*10 +data[sz11-2]-'0';
    ...
    else
    {
        ...
        for(;lc<nb;lc++)
        {
            ...
            newItable[(j*40)+ 4 + (lc*4) + 0] = inode1[0];
            newItable[(j*40)+ 4 + (lc*4) + 1] = inode1[1];
            newItable[(j*40)+ 4 + (lc*4) + 2] = inode1[2];
            newItable[(j*40)+ 4 + (lc*4) + 3] = inode1[3];
        }
        fclose(fp);
        ...
        char nndata[sz1+nz+40960+1];
        
        for(lc = 0;lc<sz1+nz;lc++)
        nndata[lc] = newData[lc];
        int jjk=0;
        for(lc = 0;lc<40960;lc++)
        nndata[sz1+nz+lc] = newItable[lc];
        
        nndata[sz1+nz+40960]= '\0';
        
        fp = fopen("/home/osboxes/Desktop/d52", "w");
        fputs(nndata,fp);
	...
	oz++;
       ...
       // i3++;
        ...
        for(lc = 0;lc<sz1;lc++)
        dirdata2[lc] = dirdata[lc];
        
        for(lc = 0;lc<4;lc++)
        dirdata2[sz1+lc] = inode1[lc];
...
{
    FILE *fp;
    fp = fopen("/home/osboxes/Desktop/d52", "rb+");
    if(fp == NULL) //if file does not exist, create it
    {
        fp = fopen("/home/osboxes/Desktop/d52", "wb");
...
void makeDirectory(){
    ...
    if(fp == NULL) //if file does not exist, create it
    {
        fp = fopen("/home/osboxes/Desktop/dir2", "wb");
